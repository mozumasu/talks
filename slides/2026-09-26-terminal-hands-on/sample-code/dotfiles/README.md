...
| `keybinds.lua` | `wezterm show-keys --lua` の出力から重複と `Ctrl+-` / `Ctrl+=` のフォントサイズ変更、herdr に渡す `cmd+t` を除き、Leader 系 (ペイン分割・移動・ズーム・閉じる・Workspace)、QuickSelect (`Cmd+Space`)、Claude Code 改行を足したもの。Leader キー (`Ctrl+;`) の定義もここ。`disable_default_key_bindings = true` でこのファイルのキーだけを使う |
